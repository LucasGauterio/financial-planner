  ...
  <div class="income-tracker-container">
    ...
    <div class="horizon-ruler-container">
      <div class="horizon-ruler-header">
        <span class="horizon-ruler-label">{{ t('income.horizon') }}</span>
        <div class="horizon-ruler-badge">
          {{ t('income.horizonMonths', { months: horizonMonths }) }} / {{ horizonEndLabel }}
        </div>
      </div>
    ...
    <div class="stats-grid">
      <div class="card stat-card lent">
        ...
        <div class="stat-content">
          <span class="stat-label">{{ t('income.total') }}</span>
      ...
      <div class="card stat-card recovered">
        ...
        <div class="stat-content">
          <span class="stat-label">{{ t('income.received') }}</span>
          <span class="stat-val success">{{ formatCurrency(stats.received) }}</span>
        </div>
      </div>
      <div class="card stat-card outstanding">
        ...
        <div class="stat-content">
          <span class="stat-label">{{ t('income.pending') }}</span>
      ...
      <div class="card stat-card active-count">
        ...
        <div class="stat-content">
          <span class="stat-label">{{ t('income.sourcesCount') }}</span>
    ...
    <div v-else class="sources-grid">
      ...
      >
        ...
        <div class="source-card-footer">
          <span v-if="source.recurring && source.endMonth" class="badge recurring">{{ t('income.form.recurring') }} → {{ source.endMonth }}</span>
          <span v-else-if="source.recurring" class="badge recurring">{{ t('income.form.recurring') }}</span>
          <span v-else class="badge one-off">{{ t('income.form.startMonth') }}: {{ source.startMonth }}</span>
          ...
          <form @submit.prevent="saveSource">
            <div class="modal-body">
              ...
              <fieldset v-if="form.recurring" class="form-fieldset">
                <legend>{{ t('income.form.endMonth') }}</legend>
                  ...
                  <div v-for="entry in monthGroup.entries" :key="`${entry.sourceId}_${entry.month}`" class="income-entry-row">
                    <div class="entry-right">
                      <template v-if="entry.status !== 'pending'">
                        <span class="actual-amount-label">{{ t('income.actualAmount') }}:</span>
                        <input
                          type="number"
...
const projectionEntries = computed(() => {
  return generateIncomeProjection(sources.value, horizonMonths.value, flattenedOverrides.value);
});
...
// marked received, falling back to the expected amount for a legacy string override or
// one the user hasn't edited yet (TD-01).
...
async function toggleStatus(entry) {
  ...
  // TD-01: preserve any previously-entered actual amount across pending<->received
  ...
  source.statusOverrides[entry.month] = entry.status === 'received'
    ? { status: 'pending', actualAmount: priorActualAmount }
    : { status: 'received', actualAmount: priorActualAmount != null ? priorActualAmount : entry.expectedAmount };
