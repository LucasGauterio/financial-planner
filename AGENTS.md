...
2. **Documentation-First Policy & Automatic Design-Docs Activation**: ALL feature additions or new phases MUST start by updating documentation in `docs/`. After `/research phase NN` completes, AUTOMATICALLY activate design-docs skills (`/design-docs-prd`, `/design-docs-rfc`, `/design-docs-fdd`, `/design-docs-adr`) to update system documentation BEFORE `/plan-context NN` and implementation. NEVER ask the user if documentation should be updated — design-docs activation is mandatory and automatic.
