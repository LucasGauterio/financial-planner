...
When executing or planning a phase/task, the pipeline runs **AUTONOMOUSLY BY DEFAULT** without asking for user confirmation between stages. When `/research phase NN` completes, the agent MUST automatically activate the system design-docs workflow (`/design-docs-prd`, `/design-docs-rfc`, `/design-docs-fdd`, `/design-docs-adr`) to update system documentation BEFORE running `/plan-context NN`. NEVER ask the user if they want to update documentation — design-docs activation is mandatory and automatic before implementation. Once design-docs are updated, automatically proceed through `/plan-context NN` -> `/plan-validate NN` -> `/plan-resolve NN` (if dirty) -> `/plan-build NN` -> `/implement phase NN` without pausing.
