...
When executing or planning a phase/task, the pipeline runs **AUTONOMOUSLY BY DEFAULT** without asking for user confirmation between stages. When `/research phase NN` completes, automatically run `/plan-context NN` -> `/plan-validate NN` -> `/plan-resolve NN` (if dirty) -> `/plan-build NN` -> `/implement phase NN` without pausing. Interactive stage-by-stage review is OPT-IN ONLY when explicitly requested by the user.
