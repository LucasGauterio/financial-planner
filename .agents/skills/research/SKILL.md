...
## Closing instructions (Automatic Post-Research Workflow)

When this skill finishes generating the decisions doc for a phase:
- **AUTOMATIC DESIGN-DOCS ACTIVATION**: Immediately and automatically execute the system design-docs workflow skills (`/design-docs-prd`, `/design-docs-rfc`, `/design-docs-fdd`, `/design-docs-adr`) to update `docs/PRD.md`, `docs/RFC.md`, `docs/FDD.md`, and `docs/adrs/`. DO NOT ask the user if they want to update documentation — design-docs activation is MANDATORY and AUTOMATIC before implementation!
- **AUTOMATIC PIPELINE PROCEED**: After system design-docs are updated, automatically proceed to `/plan-context NN` -> `/plan-validate NN` -> `/plan-resolve NN` (if dirty) -> `/plan-build NN` -> `/implement phase NN` in continuous autonomous mode. The pipeline order is `plan-context → plan-validate → plan-resolve`; each stage requires the artifact produced by the previous one (`plan-resolve` aborts with "validation.md not found" if invoked directly; `plan-validate` aborts without `CONTEXT.md`). Skipping ahead to `/plan-resolve` is always wrong.
