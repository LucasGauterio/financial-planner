...
3. **Status-override handling on edit (TD-02, Option B)** — when an edit changes `startMonth`, `recurring`, or `endMonth`, `statusOverrides` is reset to `{}` after a `ConfirmDialog` warning (`showParamsConfirm`, retrofitted per [`dialog-implementation.md`](../../.claude/rules/dialog-implementation.md)), mirroring [`updateExistingLoan`'s parameter-change guard](../../src/components/LoanTracker.vue#L677-L694). Editing only `name`, `type`, or `amount` does not trigger this reset (those fields don't affect which months are generated).
