...
### 4.8 Build & Test Toolchain Upgrade (Phase 11)
- **Modules**: `package.json`, `Dockerfile`, `vite.config.js`
- **Runtime contract** _(per [ADR-016](adrs/ADR-016-full-dependency-and-runtime-upgrade.md))_: the project's minimum Node.js version is now declared via `package.json`'s `"engines": { "node": ">=24" }`, and the Docker build stage (`Dockerfile`'s `FROM node:...`) targets `node:24-alpine`. Both `vitest@5`/`jsdom@30` require this floor (`^22.12.0`/`^22.22.2` at minimum); Node `<24` (in the sense of the exact patch floors these packages declare) is unsupported going forward.
- **Test runner major bump** _(vitest 4 → 5)_: `vite.config.js`'s `test` block (`environment: 'jsdom'`, `globals: true`, `reporters`, `coverage.provider: 'v8'`) required no shape changes — verified against the Vitest 5 migration guide, whose only behavior-relevant change for this config is `clearMocks` defaulting to `true` (every test file already calls `.mockReset()` explicitly in `beforeEach`, so this is a no-op superset here). Built-in reporter output-path changes (JSON/JUnit/HTML) don't apply, since this project's `reporters` array uses the third-party `vitest-sonar-reporter` with its own explicit `outputFile`.
- **Dependency deprecation pin** _(per [BUGFIX-004](../TRACKER.md#bug-fix-traceability))_: `package.json`'s `"overrides": { "glob": "^13.0.6" }` forces every transitive `glob` resolution (currently via `@vue/test-utils → js-beautify`) to a non-deprecated version, independent of `js-beautify`'s own major version or the host machine's Node version.

### 4.9 Spreadsheet Data Export Engine (Phase 12)
- **Modules**: `src/services/spreadsheetExportService.js` (new), `src/components/DataExport.vue` (new), `src/App.vue` (new nav entry)
- **Reference pattern**: [`BackupManager.vue`](../src/components/BackupManager.vue) (screen shape, status-message convention) and its `Blob`/anchor download idiom — superseded here by SheetJS's own `XLSX.writeFile`, which performs the download itself.
- **Library** _(per [ADR-017](adrs/ADR-017-spreadsheet-data-export.md) / [technical-decisions-spreadsheet-export.md, TD-01](decisions/technical-decisions-spreadsheet-export.md))_: SheetJS Community Edition (`xlsx`), the project's first production dependency beyond `vue`.
- **Row mapping**: `spreadsheetExportService.js` exports one pure function per domain that reads the domain's persisted shape (via the existing `repository.get*()` calls already used by each tracker) and maps it to an array of flat row objects suitable for `XLSX.utils.json_to_sheet` — Income and Expense sources are expanded through their existing `generateIncomeProjection`/`generateExpenseProjection` functions so the exported rows are the same month-by-month projection entries the trackers already display, not just the raw source records; Cash Flow reuses `cashFlowCalculations.js`'s merged entries; Loans, Portfolio, and Investment Timeline map their stored arrays directly.
- **Workbook assembly** _(TD-02, Decision: Option A)_: a single exported function takes every domain's row arrays, calls `XLSX.utils.json_to_sheet` per domain, appends each as its own tab via `XLSX.utils.book_append_sheet(wb, ws, name)` onto one `XLSX.utils.book_new()` workbook (`Income`, `Expenses`, `Loans`, `Portfolio`, `Cash Flow`, `Investment Timeline`), and calls `XLSX.writeFile(wb, filename)` — no partial export, always all six domains.
- **Plaintext warning gate** _(TD-03, Decision: Option A)_: `DataExport.vue`'s export button does not call the assembly function directly; it opens a [`ConfirmDialog`](../src/components/ConfirmDialog.vue) (`showExportConfirm`) stating the download will contain unencrypted financial data, per [`dialog-implementation.md`](../.claude/rules/dialog-implementation.md) — no native `confirm()`. The assembly function only runs after the dialog's Continue action.
- **Entry point** _(TD-04, Decision: Option B)_: `App.vue` adds a `dataExport` tab rendering `DataExport.vue`, following the existing `v-else-if="activeTab === '...'"` pattern used for `loans`/`income`/`expenses`/`cashFlow`. `DataExport.vue` follows the app's established screen conventions ([`ui-pattern-consistency.md`](../.claude/rules/ui-pattern-consistency.md)): `.section-header` with title/subtitle/primary action button, `.card` layout.
