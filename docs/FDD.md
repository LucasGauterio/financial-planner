...
- **Status-override reset on parameter edit** _(TD-02, Decision: Option B)_: when an edit changes `startMonth`, `recurring`, or `endMonth`, a native `confirm()` warns the user that recorded paid/received statuses for that source will be cleared; on confirmation, `statusOverrides` is reset to `{}` before saving — mirroring [`updateExistingLoan`'s parameter-change guard](../src/components/LoanTracker.vue#L677-L694). Editing only `name`, `type`, or `amount` does not trigger this reset.
