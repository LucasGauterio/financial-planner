...
### 4.19 Spreadsheet Data Export (Phase 12)
- **Decision**: a new `src/services/spreadsheetExportService.js` maps every persisted domain — Income, Expenses, Loans, Portfolio, Cash Flow, Investment Timeline — to flat row objects and uses SheetJS (`XLSX.utils.json_to_sheet` / `book_append_sheet` / `writeFile`) to assemble and download a single multi-sheet `.xlsx` workbook, one tab per domain, always including all domains. Because the workbook must be human-readable, it necessarily contains decrypted plaintext — the app's first user-initiated plaintext data egress path — so the action is gated by a [`ConfirmDialog.vue`](../src/components/ConfirmDialog.vue) warning (per [`dialog-implementation.md`](../.claude/rules/dialog-implementation.md)) rather than firing on click. The action lives on a new dedicated `DataExport.vue` screen reachable from main navigation, kept separate from `BackupManager.vue`'s encrypted vault backup/restore concern. `xlsx` becomes the project's first production dependency beyond `vue`. See [ADR-017](adrs/ADR-017-spreadsheet-data-export.md) and [technical-decisions-spreadsheet-export.md](decisions/technical-decisions-spreadsheet-export.md) for the full alternatives considered.
...
- **Per-domain export files and user-selectable export subset**: Phase 12's spreadsheet export (§4.19) always produces a single all-domains workbook; splitting it into a zipped file per domain, or letting the user pick which domains to include, were both considered and rejected for this phase since "export all data" as one file already satisfies the requirement. Revisiting either is deferred, not part of Phase 12 scope.
- **Password-protected or encrypted spreadsheet export**: Phase 12's export (§4.19) is deliberately plaintext, mitigated only by a warning dialog, since neither SheetJS nor ExcelJS supports native Excel password protection from the browser. Wrapping the export in a password-protected archive, if ever needed, is deferred, not part of Phase 12 scope.
